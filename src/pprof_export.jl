...
function to_pprof(alloc_profile::AllocProfile
    ...
    function maybe_add_location(location_str::String)::UInt64
        return get!(locs_map, location_str) do
            loc_id = UInt64(length(locations) + 1)

            # Parse the string
            (function_name, file_name, line_number) = parse_location(location_str)
